...
$all_terms = get_the_terms(get_the_ID(), 'project_category');
?>
  ...
  <section class="relative">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-10">
      <nav class="text-sm text-slate-600 mb-4">
        ...
        <a href="<?php echo esc_url( get_post_type_archive_link('project') ); ?>" class="hover:underline">Funnel</a>
        <?php if ($primary_term): ?>
          <span class="mx-2">/</span>
  ...
  <section class="relative">
    <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <nav class="mt-8 flex items-center justify-between gap-4">
        <div class="flex-1">
          <?php $prev = get_previous_post(true, '', 'project_category'); // constrain within taxonomy ?>
          <?php if ($prev): ?>
            <a href="<?php echo esc_url( get_permalink($prev) ); ?>" class="group inline-flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-slate-900">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition group-hover:-translate-x-0.5" viewBox="0 0 24 24" fill="currentColor"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>
              <span><?php echo esc_html( get_the_title($prev) ); ?></span>
            </a>
          <?php endif; ?>
        </div>
        <div class="flex-1 text-right">
          <?php $next = get_next_post(true, '', 'project_category'); ?>
          <?php if ($next): ?>
            <a href="<?php echo esc_url( get_permalink($next) ); ?>" class="group inline-flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-slate-900">
              ...
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="currentColor"><path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6z"/></svg>
            </a>
          <?php endif; ?>
        </div>
      </nav>
  ...
  <?php if ($related && $related->have_posts()) : ?>
    <section class="relative">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-14">
        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900 mb-6">Related Funnel</h2>
            ...
            <article class="group overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200 shadow-sm hover:shadow-md transition">
              ...
              <div class="p-5">
                ...
                <p class="mt-2 text-sm text-slate-600">
                  <?php
                    $ex = get_the_excerpt();
                    if (!$ex) { $ex = wp_trim_words(wp_strip_all_tags(get_the_content('')), 22, '…'); }
                    echo esc_html($ex);
                  ?>
                </p>
              </div>
